...
class D2MoRAModel(torch.nn.Module):
    """
    ...
    **Attributes**:
            ...
            if target_module_found:
                ...
                if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                    ...
                    if self.peft_config.enable_lora is None:
                        print("8 bit lora")
                        new_module = MOELinear8bitLt(target.in_features, target.out_features, bias=bias, **kwargs)
                    else:
                    ...
                    new_module = D2MoRAMergedLinear(in_features, out_features, bias=bias, **kwargs)
    ...
    def _set_adapter_layers(self, enabled=True):
        for module in self.model.modules():
            if isinstance(module, D2MoRALayer):
...
def mark_only_lora_as_trainable(model: nn.Module, bias: str = "none") -> None:
    ...
    if bias == "none":
        ...
        for m in model.modules():
            if isinstance(m, D2MoRALayer) and hasattr(m, "bias") and m.bias is not None:
...
class D2MoRAMergedLinear(nn.Linear, D2MoRALayer):
...
if is_bnb_available():
    ...
    class MOELinear8bitLt(bnb.nn.Linear8bitLt, D2MoRALayer):
        # Lora implemented in a dense layer
        def __init__(
            self,
            in_features,
            ...
            **kwargs,
            ...
            D2MoRALayer.__init__(self, r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, merge_weights=False)
            # Actual trainable parameters
            if r > 0:
    ...
    class MergedLinear8bitLt(bnb.nn.Linear8bitLt, D2MoRALayer):
        # Lora implemented in a dense layer
        def __init__(
            self,
            in_features: int,
